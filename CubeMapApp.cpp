...
void CubeMapApp::LoadTextures()
{
	...
	texNames.push_back("FBXModel");
	std::string texFileName1 = mFBXLoader.GetTexturePath();
	std::wstring texFileName2;
	texFileName2.assign(texFileName1.begin(), texFileName1.end());
	texFilenames.push_back(texFileName2);
...
void CubeMapApp::BuildDescriptorHeaps()
{
	...
	auto fbxTex = mTextures["FBXModel"]->Resource;
	...
	srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
	srvDesc.Format = fbxTex->GetDesc().Format;
	srvDesc.Texture2D.MipLevels = fbxTex->GetDesc().MipLevels;
	md3dDevice->CreateShaderResourceView(fbxTex.Get(), &srvDesc, hDescriptor);
}
...
void CubeMapApp::BuildShapeGeometry()
{
	...
	GeometryGenerator::MeshData fbx;
	...
	UINT fbxVertexOffset = cylinderVertexOffset + (UINT)cylinder.Vertices.size();
	...
	UINT fbxIndexOffset = cylinderIndexOffset + (UINT)cylinder.Indices32.size();
	...
	fbxSubmesh.IndexCount = (UINT)mFBXLoader.GetIndices().size();
	fbxSubmesh.StartIndexLocation = fbxIndexOffset;
	fbxSubmesh.BaseVertexLocation = fbxVertexOffset;
	...
	for (size_t i = 0; i < fbxVertexs.size(); ++i, ++k)
	{
		...
		{
			vertices[k].TexC = DirectX::XMFLOAT2(0, 0);
		}
	}

	std::vector<std::uint16_t> indices;
	std::vector<std::uint16_t> fbxIndices = mFBXLoader.GetIndices();
	indices.insert(indices.end(), std::begin(box.GetIndices16()), std::end(box.GetIndices16()));
	indices.insert(indices.end(), std::begin(grid.GetIndices16()), std::end(grid.GetIndices16()));
	indices.insert(indices.end(), std::begin(sphere.GetIndices16()), std::end(sphere.GetIndices16()));
	indices.insert(indices.end(), std::begin(cylinder.GetIndices16()), std::end(cylinder.GetIndices16()));
	indices.insert(indices.end(), std::begin(fbxIndices), std::end(fbxIndices));
...
void CubeMapApp::BuildMaterials()
{
	...
	auto fbx = std::make_unique<Material>();
	fbx->Name = "fbx";
	fbx->MatCBIndex = 5;
	fbx->DiffuseSrvHeapIndex = 4;
	fbx->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	fbx->FresnelR0 = XMFLOAT3(0.1f, 0.1f, 0.1f);
	fbx->Roughness = 0.1f;
...
void CubeMapApp::BuildRenderItems()
{
	...
	XMStoreFloat4x4(&fbxRitem->World, XMMatrixTranslation(-50.0f, -80.0f, 200.0f));
	fbxRitem->TexTransform = MathHelper::Identity4x4();
	fbxRitem->ObjCBIndex = objCBIndex++;;
	fbxRitem->Mat = mMaterials["fbx"].get();
	fbxRitem->Geo = mGeometries["shapeGeo"].get();
	...
	XMStoreFloat4x4(&box2Ritem->World, XMMatrixTranslation(10.0f, 10.0f, 0.0f));
	box2Ritem->TexTransform = MathHelper::Identity4x4();
	box2Ritem->ObjCBIndex = objCBIndex++;
	box2Ritem->Mat = mMaterials["mirror0"].get();
...
void CubeMapApp::LoadFBX(const char * filePath)
{
	mFBXLoader.LoadFBX(filePath);
}
